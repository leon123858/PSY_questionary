...
class A {
	...
	_createQuestion() {
		this._question = competitor(
			[Color_Set.green, Color_Set.red],
			[this.ratio.G, this.ratio.R]
		); //change the question
	}
	_generateAnswer = (item, range_min, range_max) => {
		...
		if (range_max != undefined)
			interval =
				Math.floor(Math.random() * (range_max - range_min)) + range_min;
	...
	_analyzeData() {
		...
		let Acc =
			Math.round(((this._groupset[0] * 100) / this.game_set) * 100) / 100;
		let RT = Math.round((this._groupset[1] / this._groupset[4]) * 100) / 100;
		let FA =
			Math.round(((this._groupset[2] * 100) / (this.game_set * 0.3)) * 100) /
			100;
		let FART = Math.round((this._groupset[3] / this._groupset[2]) * 100) / 100;
		this._group = (Acc + '_' + RT + '_' + FA + '_' + FART).replaceAll(
			'NaN',
			'NA'
		);
...
class B {
	...
	_createQuestion() {
		...
		this._bee = competitor(
			[1, 0],
			[this._beenum, this.game_set - this._beenum]
		); // 1 have bee 0 no bee
	}
					...
					if (!bee) {
						if (KEY_COLOR[e.key] == color) {
							...
							quetion_Result +=
								...
								'_1_' +
								(end - start).toString() +
								'_NA_SSAcc~'; //Press_Acc_RT_SSD_SSAcc
						} else {
							quetion_Result +=
								KEY_NUM[e.key] +
								'_0_' +
	...
	_analyzeData() {
		...
		let Cp =
			Math.round(
				((this._beenum - this._groupset[3]) / this._beenum) * 100 * 100
			) / 100;
		...
		let Go_Acc =
			Math.round(
				(this._groupset[1] / (this.game_set - this._beenum)) * 100 * 100
			) / 100;
		let Go_Rt = Math.round((this._groupset[2] / this._groupset[1]) * 100) / 100;
		let NcRate =
			Math.round((this._groupset[3] / this._beenum) * 100 * 100) / 100;
...
class C {
	...
	_generateAnswer = (item, direction, range_min, range_max) => {
		...
		return new Promise((resolve) => {
			...
			function key_handler(e) {
				...
				quetion_Result +=
					end - start + '_' + group_set + '_' + direction + '_' + keydown + '~'; //RT-Acc-Direction-key
				hide(item);
				clearTimeout(timeout);
				resolve([quetion_Result, group_set]);
			}
		...
		for (let session = 0; session < this.game_set[0]; ++session) {
			...
			for (let i = 0; i < this.game_set[1]; ++i) {
				...
				for (let d = 0; d < this.ballnum; ++d) {
					if (d < ratio) {
						this.spawn_div.children[d].setAttribute(
							'direction',
				...
				flip == 0
					? (ratio = this._createQuestion(ratio, 1, i, conti))
					: (ratio = this._createQuestion(ratio, -1, i, conti));
	...
	_analyzeData() {
		this._one = this._one.slice(0, -1); //remove last~
		let AccR1 =
			Math.round(((this._groupset[0] * 100) / this.game_set[1]) * 100) / 100;
		let AccR2 =
			Math.round(((this._groupset[1] * 100) / this.game_set[1]) * 100) / 100;
		let AccR3 =
			Math.round(((this._groupset[2] * 100) / this.game_set[1]) * 100) / 100;
		let CRR1 =
			Math.round(((this._groupset[3] * 100) / this.ballnum) * 100) / 100;
		let CRR2 =
			Math.round(((this._groupset[4] * 100) / this.ballnum) * 100) / 100;
		let CRR3 =
			Math.round(((this._groupset[5] * 100) / this.ballnum) * 100) / 100;
		...
		for (let session = 0; session < this._question.length; ++session) {
			...
			for (var item of this._question[session]) {
				...
				this._one +=
					...
					1 +
					'_' +
					Math.round(timer[garbo[0]] * 100) / 100 +
					'_' +
					(item[1] + 1) +
					'_'; //size pre direction
				await this._generateAnswer(item[1] + 1, 5000).then((data) => {
					...
					this.correct == 0
						? (timer[garbo[0]] =
								timer[garbo[0]] +
								Math.abs(last_timer[garbo[0]] - timer[garbo[0]]) * 0.2 +
								5)
						: (timer[garbo[0]] =
								timer[garbo[0]] -
								Math.abs(last_timer[garbo[0]] - timer[garbo[0]]) * 0.2 -
			...
			this._acc +=
				...
				(part[5] / part[2]) * 100;
			this._groupset +=
				Math.round(part[6] * 100) / 100 +
				'_' +
				Math.round(part[7] * 100) / 100 +
				'_' +
				Math.round(part[8] * 100) / 100 +
				'_' +
				Math.round((part[3] / part[0]) * 100 * 100) / 100 +
				'_' +
				Math.round((part[4] / part[1]) * 100 * 100) / 100 +
				'_' +
				Math.round((part[5] / part[2]) * 100 * 100) / 100 +
				'_';
			//console.log(this._groupset);
			if (session + 1 < this.game_set[0]) {
				await new Promise((resolve) => {
...
class E {
			...
			function key_handler(e) {
				...
				if (KEY_NUM[e.key] != undefined) {
					if (KEY_NUM[e.key] == (direction + 1).toString()) {
						quetion_Result +=
							KEY_NUM[e.key] + '_1_' + (end - start).toString() + '~'; //press-acc-rt
						group_set = [1, end - start];
					} else {
						quetion_Result +=
							KEY_NUM[e.key] + '_0_' + (end - start).toString() + '~'; //press-acc-rt
	...
	async process() {
		for (var item of this._question) {
			...
			this._one +=
				...
				'_'; //cue-con-pos-ori-
			//nci + right or left
			this.arrowplace[item[1] + item[3] * 3].setAttribute(
				'side',
				this.SIDE[item[2]]
			);
			await this._generateAnswer(
				this.arrowplace[item[1] + item[3] * 3],
				item[3],
				1700
			).then((data) => {
				//1700
	...
	_analyzeData() {
		this._one = this._one.slice(0, -1).replaceAll('NaN', 'NA'); //remove last~
		let Acc =
			Math.round(((this._group_num[0] * 100) / this.game_set) * 100) / 100;
...
class F {
	...
	_generateAnswer = (item, targetposition, range_min, range_max) => {
		...
		return new Promise((resolve) => {
			...
			function key_handler(e) {
				...
				if (KEY_NUM[e.key] != undefined) {
					targetposition == Direction_Num[e.key]
						? (group_set = [1, end - start])
						: (group_set = [0, end - start]);
					if (group_set[0] == 0) {
	...
	_analyzeData() {
		...
		let Soa200 =
			Math.round((this._groupset[4] / this._group_num[4]) * 100) / 100;
		let Soa1200 =
			Math.round((this._groupset[5] / this._group_num[5]) * 100) / 100;
		this._group = (
			Acc +
			'_' +
			RT +
		...
		this._pr = (Acc + '_' + RT + '_' + Ne + '_' + Co + '_' + Ico).replaceAll(
			'NaN',
			'NA'
		);
		result_label.textContent = '測驗結束: 正確率' + Acc + '%';
...
class G {
			...
			for (let length = 0; length < this.game_set; ++length) {
				...
				await new Promise((resolve) => {
					...
					function mouseclick(event) {
						...
						if (
							event.target.tagName == 'TARGET' &&
							event.target.classList.contains('enable_click')
						) {
							partscore++;
	...
	_analyzeData() {
		...
		this._one = this._one.slice(0, -1).replaceAll('NaN', 'NA'); //remove last~
		let Acc =
			Math.round(((this._score * 100) / ((this._level - 5) * 4 * 3)) * 100) /
			100;
		this._group = (this._level + '_' + this._score + '_' + Acc).replaceAll(
			'NaN',
			'NA'
		);
...
class H {
	...
	_createQuestion() {
		...
		switch (this.round) {
			case 1:
				this._question = [competitor([Color_Set.green], this.ratio[0])];
				break;
			case 2:
				this._question = [
					competitor([Color_Set.green, Color_Set.red], this.ratio[1]),
	...
	async process() {
		for (let session = 0; session < this._question.length; ++session) {
			...
			this._groupset[session] =
				(numbertimeset[0] * 100) / this.game_set[session]; //Acc
	...
	_createQuestion() {
		for (var question = 0; question < this.ratio.length; ++question) {
			for (var len = 0; len < this.each_game; ++len) {
				...
				for (
					var number_len = 0;
					number_len < this.ratio[question];
					++number_len
				) {
		...
		return new Promise((resolve) => {
			...
			function stopRun() {
				...
				for (let i = 0; i < length; ++i) {
					...
					if (keylist.length >= i) {
						let right = 0;
						if (keylist[i] == numberlist[i])
							//set not finish system
							right = 1;
			...
			function key_handler(e) {
				if (typenum >= length && e.key == 'Enter') {
					...
					(typenum < length && e.which >= 96 && e.which <= 105) ||
...
class J {
	...
	async process() {
		while (1) {
			...
			for (var item of this._question) {
				...
				await this._generateAnswer(element, item[1], 1500).then((data) => {
					//1500
	...
	_analyzeData() {
		...
		let Acc =
			Math.round(
				((this._total * 100) / (this.game_set * (this._level - 1))) * 100
			) / 100;
...
class K {
	...
	async process() {
		for (var number in this.question) {
			...
			await this._generateAnswer(this.word, 3000).then((data) => {
				...
				this._groupset = this._groupset.map(function (num, idx) {
					//add the group
					return num + data[1][idx];
				});
				this._group_time = this._group_time.map(function (num, idx) {
					//add the group_time
	...
	_analyzeData() {
		...
		let Positive =
			Math.round((this._groupset[2] / this._group_time[0]) * 100) / 100;
		let Negative =
			Math.round((this._groupset[3] / this._group_time[1]) * 100) / 100;
		let Middle =
			Math.round((this._groupset[4] / this._group_time[2]) * 100) / 100;
		this._group = (
			Acc +
			'_' +
			RT +
...
function competitor(object_list, ratio) {
	...
	if (typeof ratio === 'number') {
		for (number = 0; number < object_list.length; ++number) {
			for (var i = 0; i < ratio; ++i) arr.push(object_list[number]);
		}
	} else {
		for (var number in ratio) {
			let each_length = ratio[number];
			for (i = 0; i < each_length; ++i) arr.push(object_list[number]);
		}
	}
	arr = shuffle(arr);
	return arr;
