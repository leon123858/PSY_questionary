		...
		questions.map((value, index) => {
			switch (value) {
				case TYPE.TEN:
					...
					break;
				//'/image/CQ_P_picture/1.png', <= real environment
				case TYPE.IMAGE:
					const randomInt = generateRandomInt(1, 9);
					this._tmpImgList.push(randomInt);
					timeline.push({
						type: 'image-keyboard-response',
						stimulus: `/image/CQ_P_picture/${randomInt}.png`,
						choices: jsPsych.NO_KEYS,
						stimulus_width: 100,
				...
				case TYPE.SURVEY:
					timeline.push({
						...
						html: `<label id="userInput">${lowLine(
							level
						)}</label><input id="userAns" name="response" value="" style="display:none" disable><br><img id="list" src="/image/CQ_P_picture/list.jpg">`,
					});
					break;
				case TYPE.SCORE:
					timeline.push({
		...
		const labelAddWord = (value, key) => {
			...
			for (let i = 0; i < value.length; i++) {
				if (value[i] == '_') {
					input.value += key[0];
					$('#userInput').before(
						`<img class="obj" id="ansImg" src="/image/CQ_P_picture/${key[0]}.png">`
					);
